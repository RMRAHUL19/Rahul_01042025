...
public void donavigatewardUnit() {
	IMWU=hishomepage.doNavigateTogetwardUnitPage();
   	...
   	@Test(priority = 4)
   	@Parameters("browser")
   	public void save(String Browser) throws InterruptedException {
   	...
   	//Selecting the date of surgery	
		...
		Thread.sleep(3000);
   	   ...
   	   wait.until(ExpectedConditions.visibilityOf(IMWU.getDoctorCheckBox()));
   		Thread.sleep(3000);
   		utilobj.selectDropdown(IMWU.getHeadofUnit(), driver, "TestNurse for ESICTestHospital",Browser);//select unit of head
   		Thread.sleep(3000);
	  ...
	  @Test(priority = 5)
	@Parameters("browser") 
	  public void WardUnit(String Browser) throws InterruptedException, AWTException {
		   //Selecting the date of surgery	
				SimpleDateFormat sdf = new SimpleDateFormat("dd-MMMM-yyyy");
				//Getting current date
				Calendar cal = Calendar.getInstance();
               ...
               wait.until(ExpectedConditions.visibilityOf(IMWU.getUnitNameicon()));
				...
				IMWU.getBed().click();
				Thread.sleep(3000);
				utilobj.selectDropdown(IMWU.getddlward(), driver, "Female medical ward",Browser);
				Thread.sleep(3000);
